...
from broker_runtime.sse_done import DONE_SSE_FRAME, SseDoneTracker, content_type_is_sse
...
async def _open_upstream(request: "web.Request", rel_path: str, body: bytes, cred: UpstreamCredential):
    ...
    # Adapter-owned identity/account headers override untrusted client values.
    # Authorization is assigned last so neither side can replace the broker bearer.
    for name, value in cred.extra_headers.items():
        if str(name).lower() not in _HOP_BY_HOP_HEADERS:
            fwd_headers[str(name)] = str(value)
